...
export default function ORGHeadOfficeList() {
  ...
  const columns: ColumnsType<DepartmentTree> = [
...

    {
      ...
      render: (value, record, index) => (
        <TableActions
          allowActions={record.level === 3 ? ['edit', 'switch'] : ['edit', 'switch', 'add']}
  ...
  const handleClickEdit = (record: DepartmentTree) => {
    navigate({
      ...
      search: createSearchParams({ ...currentParams, org_name: record.name, layer: `${record.level}` }).toString(),
    })
  }
  ...
  const handleColumnAction = (action, value, record: Department) => {
    ...
    if (action === EActionsType.add) {
      ...
      const parentDepartment = departments.find((department) => department.id === record.parentId)
      ...
      navigate({
        ...
        search: createSearchParams({
          ...
          layer:
            record.level === 1
              ? `${EOrganizationListLayer.layer_02}-${record.id}`
              : `${EOrganizationListLayer.layer_03}-${parentDepartment.id}-${record.id}`,
        }).toString(),
      })
    }
    return
